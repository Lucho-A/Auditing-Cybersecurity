...
int ssh(int type){
	...
	case SSH_FINGER_PRINTING:
		...
		printf("  Banner: ");
		show_message((char *) libssh2_session_banner_get(sshSessionConn),strlen((char *) libssh2_session_banner_get(sshSessionConn)), 0, INFO_MESSAGE, false, false, false);
		size_t len;
		int type;
		...
		if(hostkey!=NULL){
			...
			printf("\n\n  Algorithm: ");
			show_message(hostkeyTypeMsg,strlen(hostkeyTypeMsg), 0, INFO_MESSAGE,false, false, false);
		}
		...
		show_message(userauthlist,strlen(userauthlist), 0, INFO_MESSAGE,false, false, false);
		libssh2_session_free(sshSessionConn);
		break;
	case SSH_USER_ENUM:
