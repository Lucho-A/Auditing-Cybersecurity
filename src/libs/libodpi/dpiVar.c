...
int dpiVar__allocate(dpiConn *conn, dpiOracleTypeNum oracleTypeNum,
	...
	*var = NULL;
	...
	if (type->sizeInBytes)
		...
		sizeInBytes = size * conn->env->nmaxBytesPerCharacter;

	// allocate memory for variable type
	if (dpiGen__allocate(DPI_HTYPE_VAR, conn->env, (void**) &tempVar, error)
			< 0)
		return DPI_FAILURE;
...
static int dpiVar__allocateDynamicBytes(dpiDynamicBytes *dynBytes,
	...
	if (size > dynBytes->chunks->allocatedLength) {
		...
		dynBytes->chunks->allocatedLength = (size + DPI_DYNAMIC_BYTES_CHUNK_SIZE
				- 1) & ~(DPI_DYNAMIC_BYTES_CHUNK_SIZE - 1);
...
//-----------------------------------------------------------------------------
static int dpiVar__checkArraySize(dpiVar *var, uint32_t pos, const char *fnName,
		dpiError *error) {
	...
	if (pos >= var->buffer.maxArraySize)
		return dpiError__set(error, "check array size",
				DPI_ERR_INVALID_ARRAY_POSITION, pos, var->buffer.maxArraySize);
	return DPI_SUCCESS;
}
...
int dpiVar__convertToLob(dpiVar *var, dpiError *error) {
	...
	if (var->type->oracleTypeNum == DPI_ORACLE_TYPE_RAW
		...
		var->type = dpiOracleType__getFromNum(DPI_ORACLE_TYPE_NCLOB, error);
	else
		var->type = dpiOracleType__getFromNum(DPI_ORACLE_TYPE_CLOB, error);
...
int dpiVar__copyData(dpiVar *var, uint32_t pos, dpiData *sourceData,
	...
	case DPI_NATIVE_TYPE_BYTES:
		return dpiVar__setFromBytes(var, pos, sourceData->value.asBytes.ptr,
				sourceData->value.asBytes.length, error);
	case DPI_NATIVE_TYPE_JSON:
		return dpiVar__setFromJson(var, pos, sourceData->value.asJson, error);
	case DPI_NATIVE_TYPE_LOB:
		return dpiVar__setFromLob(var, pos, sourceData->value.asLOB, error);
	case DPI_NATIVE_TYPE_OBJECT:
		return dpiVar__setFromObject(var, pos, sourceData->value.asObject,
				error);
	case DPI_NATIVE_TYPE_STMT:
		return dpiVar__setFromStmt(var, pos, sourceData->value.asStmt, error);
	case DPI_NATIVE_TYPE_ROWID:
		return dpiVar__setFromRowid(var, pos, sourceData->value.asRowid, error);
...
//-----------------------------------------------------------------------------
int dpiVar__extendedPreFetch(dpiVar *var, dpiVarBuffer *buffer, dpiError *error) {
	dpiVector *vector;
	dpiRowid *rowid;
	dpiData *data;
	dpiStmt *stmt;
	...
	case DPI_ORACLE_TYPE_STMT:
		for (i = 0; i < buffer->maxArraySize; i++) {
			...
			if (buffer->references[i].asStmt) {
				dpiGen__setRefCount(buffer->references[i].asStmt, error, -1);
	...
	case DPI_ORACLE_TYPE_BFILE:
		for (i = 0; i < buffer->maxArraySize; i++) {
			...
			if (buffer->references[i].asLOB) {
				dpiGen__setRefCount(buffer->references[i].asLOB, error, -1);
	...
	case DPI_ORACLE_TYPE_ROWID:
		for (i = 0; i < buffer->maxArraySize; i++) {
			...
			if (buffer->references[i].asRowid) {
				dpiGen__setRefCount(buffer->references[i].asRowid, error, -1);
	...
	case DPI_ORACLE_TYPE_OBJECT:
		for (i = 0; i < buffer->maxArraySize; i++) {
			...
			if (buffer->references[i].asObject) {
				dpiGen__setRefCount(buffer->references[i].asObject, error, -1);
	...
	case DPI_ORACLE_TYPE_JSON:
		for (i = 0; i < buffer->maxArraySize; i++) {
			...
			if (buffer->references[i].asJson) {
				dpiGen__setRefCount(buffer->references[i].asJson, error, -1);
	...
	case DPI_ORACLE_TYPE_VECTOR:
		for (i = 0; i < buffer->maxArraySize; i++) {
			...
			if (buffer->references[i].asVector) {
				dpiGen__setRefCount(buffer->references[i].asVector, error, -1);
...
int dpiVar__getValue(dpiVar *var, dpiVarBuffer *buffer, uint32_t pos,
	...
	if (data->isNull) {
		if (inFetch && var->objectType && var->objectType->isCollection) {
			if (dpiOci__objectFree(var->env->handle, buffer->data.asObject[pos],
					1, error) < 0)
				return DPI_FAILURE;
	...
	if (buffer->returnCode) {
		if (buffer->returnCode[pos] != 0) {
			dpiError__set(error, "check return code", DPI_ERR_COLUMN_FETCH, pos,
					buffer->returnCode[pos]);
	...
	case DPI_NATIVE_TYPE_DOUBLE:
		switch (oracleTypeNum) {
		case DPI_ORACLE_TYPE_NUMBER:
			return dpiDataBuffer__fromOracleNumberAsDouble(&data->value, error,
					&buffer->data.asNumber[pos]);
		case DPI_ORACLE_TYPE_NATIVE_DOUBLE:
			data->value.asDouble = buffer->data.asDouble[pos];
			return DPI_SUCCESS;
		case DPI_ORACLE_TYPE_DATE:
			return dpiDataBuffer__fromOracleDateAsDouble(&data->value, var->env,
					error, &buffer->data.asDate[pos]);
		case DPI_ORACLE_TYPE_TIMESTAMP:
		case DPI_ORACLE_TYPE_TIMESTAMP_TZ:
		case DPI_ORACLE_TYPE_TIMESTAMP_LTZ:
			return dpiDataBuffer__fromOracleTimestampAsDouble(&data->value,
					oracleTypeNum, var->env, error,
	...
	case DPI_NATIVE_TYPE_BYTES:
		...
		case DPI_ORACLE_TYPE_XMLTYPE:
			...
			if (buffer->actualLength16)
				...
				bytes->length = buffer->actualLength32[pos];
			return DPI_SUCCESS;
		case DPI_ORACLE_TYPE_CLOB:
		case DPI_ORACLE_TYPE_NCLOB:
		case DPI_ORACLE_TYPE_BLOB:
		case DPI_ORACLE_TYPE_BFILE:
			return dpiVar__setBytesFromLob(bytes, &buffer->dynamicBytes[pos],
					buffer->references[pos].asLOB, error);
		case DPI_ORACLE_TYPE_NUMBER:
			bytes->length = DPI_NUMBER_AS_TEXT_CHARS;
			if (var->env->charsetId == DPI_CHARSET_ID_UTF16)
				bytes->length *= 2;
			return dpiDataBuffer__fromOracleNumberAsText(&data->value, var->env,
					error, &buffer->data.asNumber[pos]);
		default:
			break;
		}
	...
	case DPI_NATIVE_TYPE_TIMESTAMP:
		...
		return dpiDataBuffer__fromOracleTimestamp(&data->value, var->env, error,
				buffer->data.asTimestamp[pos],
	...
	case DPI_NATIVE_TYPE_OBJECT:
		...
		if (!buffer->references[pos].asObject) {
			if (dpiObject__allocate(var->objectType, buffer->data.asObject[pos],
...
UNUSED uint32_t iter, UNUSED uint32_t index, void **bufpp, uint32_t *alenp,
...
static int dpiVar__initBuffer(dpiVar *var, dpiVarBuffer *buffer,
	...
	if (!var->isDynamic && !buffer->actualLength16 && !buffer->actualLength32) {
		if (var->env->versionInfo->versionNum < 12 && buffer == &var->buffer) {
			if (dpiUtils__allocateMemory(buffer->maxArraySize, sizeof(uint16_t),
					0, "allocate actual length",
					(void**) &buffer->actualLength16, error) < 0)
				return DPI_FAILURE;
			for (i = 0; i < buffer->maxArraySize; i++)
				buffer->actualLength16[i] = (uint16_t) var->sizeInBytes;
		} else {
			if (dpiUtils__allocateMemory(buffer->maxArraySize, sizeof(uint32_t),
					0, "allocate actual length",
	...
	if (var->type->defaultNativeTypeNum == DPI_NATIVE_TYPE_BYTES
		...
		if (dpiUtils__allocateMemory(buffer->maxArraySize, sizeof(uint16_t), 0,
				"allocate return code", (void**) &buffer->returnCode, error)
				< 0)
			return DPI_FAILURE;
	}
	...
	if (!buffer->externalData) {
		if (dpiUtils__allocateMemory(buffer->maxArraySize, sizeof(dpiData), 1,
				"allocate external data", (void**) &buffer->externalData, error)
				< 0)
			return DPI_FAILURE;
		for (i = 0; i < buffer->maxArraySize; i++)
	...
	if (var->nativeTypeNum == DPI_NATIVE_TYPE_BYTES) {
		for (i = 0; i < buffer->maxArraySize; i++) {
			...
			if (var->type->charsetForm == DPI_SQLCS_IMPLICIT)
				...
				bytes->encoding = var->env->nencoding;
			if (buffer->tempBuffer)
	...
	case DPI_ORACLE_TYPE_OBJECT:
		...
		if (dpiUtils__allocateMemory(buffer->maxArraySize, sizeof(void*), 0,
				"allocate object indicator", (void**) &buffer->objectIndicator,
				error) < 0)
			return DPI_FAILURE;
		return dpiVar__extendedPreFetch(var, buffer, error);
	default:
...
int32_t dpiVar__outBindCallback(dpiVar *var, void *bindp, UNUSED uint32_t iter,
	...
	if (index == 0) {
		...
		DPI_OCI_ATTR_ROWS_RETURNED, "get rows returned", var->error) < 0)
			return DPI_OCI_ERROR;
...
static int dpiVar__setBytesFromLob(dpiBytes *bytes, dpiDynamicBytes *dynBytes,
	...
	if (dpiVar__allocateDynamicBytes(dynBytes, (uint32_t) lengthInBytes, error)
			< 0)
		return DPI_FAILURE;
	...
	if (length > 0
			&& dpiLob__readBytes(lob, 1, length, dynBytes->chunks->ptr,
...
static int dpiVar__setFromBytes(dpiVar *var, uint32_t pos, const char *value,
	...
	if ((var->buffer.tempBuffer && var->env->charsetId == DPI_CHARSET_ID_UTF16
...
static int dpiVar__setFromVector(dpiVar *var, uint32_t pos, dpiVector *vector,
	...
	if (dpiGen__checkHandle(vector, DPI_HTYPE_VECTOR, "check vector", error)
			< 0)
		return DPI_FAILURE;
...
int dpiVar__setValue(dpiVar *var, dpiVarBuffer *buffer, uint32_t pos,
	...
	case DPI_NATIVE_TYPE_DOUBLE:
		...
		case DPI_ORACLE_TYPE_NATIVE_DOUBLE:
			...
			return DPI_SUCCESS;
		case DPI_ORACLE_TYPE_NUMBER:
			return dpiDataBuffer__toOracleNumberFromDouble(&data->value, error,
					&buffer->data.asNumber[pos]);
		case DPI_ORACLE_TYPE_DATE:
			return dpiDataBuffer__toOracleDateFromDouble(&data->value, var->env,
					error, &buffer->data.asDate[pos]);
		case DPI_ORACLE_TYPE_TIMESTAMP:
		case DPI_ORACLE_TYPE_TIMESTAMP_TZ:
		case DPI_ORACLE_TYPE_TIMESTAMP_LTZ:
	...
	case DPI_NATIVE_TYPE_BYTES:
		if (oracleTypeNum == DPI_ORACLE_TYPE_NUMBER)
			return dpiDataBuffer__toOracleNumberFromText(&data->value, var->env,
					error, &buffer->data.asNumber[pos]);
		if (buffer->actualLength32)
			buffer->actualLength32[pos] = data->value.asBytes.length;
		else if (buffer->actualLength16)
			buffer->actualLength16[pos] = (uint16_t) data->value.asBytes.length;
		if (buffer->returnCode)
	...
	case DPI_NATIVE_TYPE_TIMESTAMP:
		if (oracleTypeNum == DPI_ORACLE_TYPE_DATE)
			...
			return dpiDataBuffer__toOracleTimestamp(&data->value, var->env,
					error, buffer->data.asTimestamp[pos], 0);
		else if (oracleTypeNum == DPI_ORACLE_TYPE_TIMESTAMP_TZ
				|| oracleTypeNum == DPI_ORACLE_TYPE_TIMESTAMP_LTZ)
			return dpiDataBuffer__toOracleTimestamp(&data->value, var->env,
					error, buffer->data.asTimestamp[pos], 1);
		break;
	case DPI_NATIVE_TYPE_INTERVAL_DS:
		return dpiDataBuffer__toOracleIntervalDS(&data->value, var->env, error,
				buffer->data.asInterval[pos]);
	case DPI_NATIVE_TYPE_INTERVAL_YM:
		return dpiDataBuffer__toOracleIntervalYM(&data->value, var->env, error,
				buffer->data.asInterval[pos]);
	case DPI_NATIVE_TYPE_BOOLEAN:
	...
	case DPI_NATIVE_TYPE_STMT:
		...
		DPI_OCI_HTYPE_STMT, &data->value.asStmt->prefetchRows,
				...
				DPI_OCI_ATTR_PREFETCH_ROWS, "set prefetch rows for REF cursor",
				error);
	default:
		break;
	}
...
int dpiVar_getNumElementsInArray(dpiVar *var, uint32_t *numElements) {
	...
	if (var->dynBindBuffers)
		...
		*numElements = var->buffer.actualArraySize;
