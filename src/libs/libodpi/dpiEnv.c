...
int dpiEnv__getBaseDate(dpiEnv *env, uint32_t dataType, void **baseDate,
	...
	if (!*storedBaseDate) {
		if (dpiOci__descriptorAlloc(env->handle, storedBaseDate, descriptorType,
				"alloc base date descriptor", error) < 0)
			return DPI_FAILURE;
...
int dpiEnv__init(dpiEnv *env, const dpiContext *context,
	...
	if (externalHandle) {
		...
		// lookup encoding
		if (params->encoding
				&& dpiGlobal__lookupCharSet(params->encoding, &env->charsetId,
						error) < 0)
			return DPI_FAILURE;
		...
		if (params->nencoding && params->encoding
			...
			env->ncharsetId = env->charsetId;
				...
				&& dpiGlobal__lookupCharSet(params->nencoding, &env->ncharsetId,
						error) < 0)
			return DPI_FAILURE;
