...
int dpiSodaColl__allocate(dpiSodaDb *db, void *handle, dpiSodaColl **coll,
	...
	if (dpiOci__attrGet(handle, DPI_OCI_HTYPE_SODA_COLLECTION, (void*) &sqlType,
			0, DPI_OCI_ATTR_SODA_CTNT_SQL_TYPE, "get content sql type", error)
			< 0)
		return DPI_FAILURE;
...
static int dpiSodaColl__createOperOptions(dpiSodaColl *coll,
	...
	DPI_OCI_HTYPE_SODA_OPER_OPTIONS, "allocate SODA operation options handle",
			error) < 0)
		return DPI_FAILURE;
...
static int dpiSodaColl__find(dpiSodaColl *coll,
	...
	if (dpiSodaColl__createOperOptions(coll, options, &optionsHandle, error)
			< 0)
		return DPI_FAILURE;
...
static int dpiSodaColl__insertMany(dpiSodaColl *coll, uint32_t numDocs,
	...
	DPI_OCI_HTYPE_SODA_OUTPUT_OPTIONS, "allocate SODA output options handle",
			error) < 0)
		return DPI_FAILURE;
	...
	if (operOptionsHandle) {
		status = dpiOci__sodaBulkInsertAndGetWithOpts(coll, docHandles, numDocs,
				operOptionsHandle, outputOptionsHandle, mode, error);
...
int dpiSodaColl__listIndexes(dpiSodaColl *coll, uint32_t flags,
	...
	if (dpiUtils__checkClientVersionMulti(coll->env->versionInfo, 19, 13, 21, 3,
			error) < 0)
		return DPI_FAILURE;
...
static int dpiSodaColl__populateOperOptions(dpiSodaColl *coll,
	...
	if (options->fetchArraySize > 0) {
		if (dpiUtils__checkClientVersion(coll->env->versionInfo, 19, 5, error)
				< 0)
			return DPI_FAILURE;
		if (dpiOci__attrSet(handle, DPI_OCI_HTYPE_SODA_OPER_OPTIONS,
...
//-----------------------------------------------------------------------------
static int dpiSodaColl__save(dpiSodaColl *coll, dpiSodaDoc *doc, uint32_t flags,
		dpiSodaDoc **savedDoc, void *optionsHandle, dpiError *error) {
	void *docHandle;
	uint32_t mode;
	int status;
	...
	if (!savedDoc) {
		...
		*savedDoc = NULL;
		...
		if (status == 0 && docHandle) {
			status = dpiSodaDoc__allocate(coll->db, docHandle, savedDoc, error);
			if (status < 0)
				dpiOci__handleFree(docHandle, DPI_OCI_HTYPE_SODA_DOCUMENT);
		}
	}
...
int dpiSodaColl_insertManyWithOptions(dpiSodaColl *coll, uint32_t numDocs,
	...
	if (dpiUtils__checkClientVersion(coll->env->versionInfo, 18, 5, &error) < 0)
		return dpiGen__endPublicFn(coll, DPI_FAILURE, &error);
...
int dpiSodaColl_insertOne(dpiSodaColl *coll, dpiSodaDoc *doc, uint32_t flags,
	...
	return dpiSodaColl_insertOneWithOptions(coll, doc, NULL, flags, insertedDoc);
}
...
int dpiSodaColl_insertOneWithOptions(dpiSodaColl *coll, dpiSodaDoc *doc,
	...
	if (dpiSodaColl__check(coll, __func__, &error) < 0)
		return dpiGen__endPublicFn(coll, DPI_FAILURE, &error);
	if (dpiGen__checkHandle(doc, DPI_HTYPE_SODA_DOC, "check document", &error)
			< 0)
		return dpiGen__endPublicFn(coll, DPI_FAILURE, &error);
...
//-----------------------------------------------------------------------------
int dpiSodaColl_replaceOne(dpiSodaColl *coll, const dpiSodaOperOptions *options,
		dpiSodaDoc *doc, uint32_t flags, int *replaced,
...
int dpiSodaColl_saveWithOptions(dpiSodaColl *coll, dpiSodaDoc *doc,
	...
	if (dpiUtils__checkClientVersion(coll->env->versionInfo, 19, 9, &error) < 0)
		return dpiGen__endPublicFn(coll, DPI_FAILURE, &error);
...
int dpiSodaColl_truncate(dpiSodaColl *coll) {
	...
	if (dpiUtils__checkClientVersion(coll->env->versionInfo, 20, 1, &error) < 0)
		return dpiGen__endPublicFn(coll, DPI_FAILURE, &error);
