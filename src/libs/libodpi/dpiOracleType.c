...
//-----------------------------------------------------------------------------
static const dpiOracleType dpiAllOracleTypes[DPI_ORACLE_TYPE_MAX
		- DPI_ORACLE_TYPE_NONE - 1] = { {
...
int dpiOracleType__populateTypeInfo(dpiConn *conn, void *handle,
	...
	if (handleType == DPI_OCI_DTYPE_PARAM)
		...
		dataTypeAttribute = DPI_OCI_ATTR_DATA_TYPE;
	...
	if (info->ociTypeCode != DPI_SQLT_CHR && info->ociTypeCode != DPI_SQLT_AFC
	...
	case DPI_NATIVE_TYPE_INTERVAL_DS:
		...
		DPI_OCI_ATTR_SCALE, "get scale", error) < 0)
			return DPI_FAILURE;
		if (dpiOci__attrGet(handle, handleType, (void*) &info->precision, 0,
				DPI_OCI_ATTR_PRECISION, "get precision", error) < 0)
			return DPI_FAILURE;
	...
	if (info->oracleTypeNum == DPI_ORACLE_TYPE_OBJECT) {
		if (dpiObjectType__allocate(conn, handle, handleType, &info->objectType,
				error) < 0)
			return DPI_FAILURE;
	...
	if (handleType == DPI_OCI_HTYPE_DESCRIBE
		...
		if (info->numAnnotations > 0) {
			...
			if (dpiOci__attrGet(handle, handleType, (void*) &listParam, 0,
					DPI_OCI_ATTR_LIST_ANNOTATIONS, "get annotation list param",
					error) < 0)
				return DPI_FAILURE;
			...
			for (i = 0; i < info->numAnnotations; i++) {
				...
				if (dpiOci__paramGet(listParam, DPI_OCI_DTYPE_PARAM, &itemParam,
						(uint32_t) i + 1, "get annotation", error) < 0)
					return DPI_FAILURE;
				if (dpiOci__attrGet(itemParam, DPI_OCI_DTYPE_PARAM,
						(void*) &annotation->key, &annotation->keyLength,
						DPI_OCI_ATTR_ANNOTATION_KEY, "get annotation key",
						error) < 0)
					return DPI_FAILURE;
				if (dpiOci__attrGet(itemParam, DPI_OCI_DTYPE_PARAM,
						(void*) &annotation->value, &annotation->valueLength,
						DPI_OCI_ATTR_ANNOTATION_VALUE, "get annotation value",
						error) < 0)
					return DPI_FAILURE;
			}
	...
	if (info->oracleTypeNum == DPI_ORACLE_TYPE_VECTOR) {
		...
		DPI_OCI_ATTR_VECTOR_DATA_FORMAT, "get vector column format", error) < 0)
			return DPI_FAILURE;
		...
		DPI_OCI_ATTR_VECTOR_PROPERTY, "get vector column flags", error) < 0)
			return DPI_FAILURE;
