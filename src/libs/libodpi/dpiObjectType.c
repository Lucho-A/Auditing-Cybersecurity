...
static int dpiObjectType__check(dpiObjectType *objType, const char *fnName,
		dpiError *error) {
	if (dpiGen__startPublicFn(objType, DPI_HTYPE_OBJECT_TYPE, fnName, error)
			< 0)
		return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
static int dpiObjectType__describe(dpiObjectType *objType, void *describeHandle,
		dpiError *error) {
	...
	DPI_OCI_ATTR_NAME, &objType->name, &objType->nameLength, error) < 0)
		return DPI_FAILURE;

	// determine the package name of the type
	if (dpiUtils__getAttrStringWithDup("get package name", param,
	DPI_OCI_DTYPE_PARAM, DPI_OCI_ATTR_PACKAGE_NAME, &objType->packageName,
			&objType->packageNameLength, error) < 0)
		return DPI_FAILURE;
	...
	if (typeCode == DPI_SQLT_NCO) {
		...
		DPI_OCI_ATTR_COLLECTION_ELEMENT, "get collection descriptor", error)
				< 0)
			return DPI_FAILURE;
...
int dpiObjectType_getAttributes(dpiObjectType *objType, uint16_t numAttributes,
	...
	for (i = 0; i < objType->numAttributes; i++) {
		...
		if (dpiObjectAttr__allocate(objType, attrParam, &attributes[i], &error)
				< 0) {
			dpiOci__handleFree(describeHandle, DPI_OCI_HTYPE_DESCRIBE);
			return dpiGen__endPublicFn(objType, DPI_FAILURE, &error);
		}
...
int dpiObjectType_getInfo(dpiObjectType *objType, dpiObjectTypeInfo *info) {
	...
	if (dpiGen__startPublicFn(objType, DPI_HTYPE_OBJECT_TYPE, __func__, &error)
			< 0)
		return dpiGen__endPublicFn(objType, DPI_FAILURE, &error);
