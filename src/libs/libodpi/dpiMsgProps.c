...
int dpiMsgProps__allocate(dpiConn *conn, dpiMsgProps **props, dpiError *error) {
	...
	DPI_OCI_DTYPE_AQMSG_PROPERTIES, "allocate descriptor", error) < 0) {
...
int dpiMsgProps__setRecipients(dpiMsgProps *props, dpiMsgRecipient *recipients,
	...
	for (i = 0; i < numRecipients; i++) {
		...
		DPI_OCI_DTYPE_AQAGENT, "allocate agent descriptor", error) < 0)
			return DPI_FAILURE;
	...
	if (dpiOci__attrSet(props->handle, DPI_OCI_DTYPE_AQMSG_PROPERTIES, aqAgents,
			numRecipients, DPI_OCI_ATTR_RECIPIENT_LIST, "set recipient list",
			error) < 0)
		return DPI_FAILURE;
...
int dpiMsgProps_getDelay(dpiMsgProps *props, int32_t *value) {
	...
	return dpiMsgProps__getAttrValue(props, DPI_OCI_ATTR_DELAY, __func__, value,
			&valueLength);
}
...
int dpiMsgProps_getEnqTime(dpiMsgProps *props, dpiTimestamp *value) {
	...
	if (dpiGen__startPublicFn(props, DPI_HTYPE_MSG_PROPS, __func__, &error) < 0)
		return dpiGen__endPublicFn(props, DPI_FAILURE, &error);
	DPI_CHECK_PTR_NOT_NULL(props, value)
...
int dpiMsgProps_getOriginalMsgId(dpiMsgProps *props, const char **value,
	...
	DPI_CHECK_PTR_NOT_NULL(props, valueLength)
	if (dpiOci__attrGet(props->handle, DPI_OCI_DTYPE_AQMSG_PROPERTIES,
			&rawValue, NULL, DPI_OCI_ATTR_ORIGINAL_MSGID, "get attribute value",
			&error) < 0)
		return dpiGen__endPublicFn(props, DPI_FAILURE, &error);
	...
	if (value && valueLength) {
		if (props->payloadRaw) {
			dpiOci__rawPtr(props->env->handle, props->payloadRaw,
					(void**) value);
			dpiOci__rawSize(props->env->handle, props->payloadRaw, valueLength);
		} else {
			*value = NULL;
			*valueLength = 0;
		}
...
int dpiMsgProps_setPayloadJson(dpiMsgProps *props, dpiJson *json) {
	...
	if (dpiGen__startPublicFn(props, DPI_HTYPE_MSG_PROPS, __func__, &error) < 0)
		return dpiGen__endPublicFn(props, DPI_FAILURE, &error);
	if (dpiGen__checkHandle(json, DPI_HTYPE_JSON, "check json object", &error)
			< 0)
		return dpiGen__endPublicFn(props, DPI_FAILURE, &error);
...
//-----------------------------------------------------------------------------
int dpiMsgProps_setRecipients(dpiMsgProps *props, dpiMsgRecipient *recipients,
