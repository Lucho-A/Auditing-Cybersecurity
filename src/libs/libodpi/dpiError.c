...
#include "../../libs/libodpi/dpiErrorMessages.h"
...
int dpiError__initHandle(dpiError *error) {
	if (dpiHandlePool__acquire(error->env->errorHandles, &error->handle, error)
			< 0)
		return DPI_FAILURE;
	if (!error->handle) {
...
int dpiError__set(dpiError *error, const char *action, dpiErrorNum errorNum,
	...
	if (error) {
		...
		error->buffer->messageLength = (uint32_t) vsnprintf(
...
int dpiError__setFromOCI(dpiError *error, int status, dpiConn *conn,
	...
	if (status == DPI_OCI_INVALID_HANDLE)
		...
		return dpiError__set(error, action, DPI_ERR_UNEXPECTED_OCI_RETURN_VALUE,
				status, error->buffer->fnName);
		...
		case 12161: // TNS:internal error: partial data received
			...
			if (conn->env->versionInfo->versionNum >= 18)
				dpiOci__attrGet(conn->handle, DPI_OCI_HTYPE_SVCCTX,
						...
						NULL, error);
			if (callTimeout > 0)
				return dpiError__wrap(error, DPI_ERR_CALL_TIMEOUT, callTimeout,
						error->buffer->code);
			break;
		}
...
int dpiError__wrap(dpiError *error, dpiErrorNum errorNum, ...) {
	...
	error->buffer->errorNum = errorNum;
	va_start(varArgs, errorNum);
	error->buffer->messageLength = (uint32_t) vsnprintf(error->buffer->message,
