...
static int dpiConn__close(dpiConn *conn, uint32_t mode, const char *tag,
	...
	if (conn->externalHandle) {
		...
		NULL) == DPI_SUCCESS && (mode & DPI_MODE_CONN_CLOSE_RETAG) && tag
				&& tagLength > 0)
...
int dpiConn__create(dpiConn *conn, const dpiContext *context,
	...
	if (createParams->externalHandle) {
		...
		if (dpiOci__attrGet(createParams->externalHandle, DPI_OCI_HTYPE_SVCCTX,
				&envHandle, NULL, DPI_OCI_ATTR_ENV, "get env handle", error)
				< 0)
			return DPI_FAILURE;
	...
	if (!pool
			&& dpiEnv__init(conn->env, context, commonParams, envHandle,
...
static int dpiConn__createStandalone(dpiConn *conn, const char *userName,
	...
	if (dpiOci__serverAttach(conn, connectString, connectStringLength, error)
			< 0)
		return DPI_FAILURE;

	// allocate the service context handle
	if (dpiOci__handleAlloc(conn->env->handle, &conn->handle,
	DPI_OCI_HTYPE_SVCCTX, "allocate service context handle", error) < 0)
		return DPI_FAILURE;
	...
	DPI_OCI_HTYPE_SESSION, userName, userNameLength, password, passwordLength,
			createParams, error) < 0)
		return DPI_FAILURE;

	// set the session handle on the service context handle
	if (dpiOci__attrSet(conn->handle, DPI_OCI_HTYPE_SVCCTX, conn->sessionHandle,
			0, DPI_OCI_ATTR_SESSION, "set session handle", error) < 0)
		return DPI_FAILURE;
...
void dpiConn__free(dpiConn *conn, dpiError *error) {
	if (conn->handle)
		dpiConn__close(conn, DPI_MODE_CONN_CLOSE_DEFAULT, NULL, 0, 0, error);
	if (conn->pool) {
...
static int dpiConn__get(dpiConn *conn, const char *userName,
	...
	DPI_OCI_HTYPE_AUTHINFO, userName, userNameLength, password, passwordLength,
			createParams, error) < 0) {
	...
	status = dpiConn__getSession(conn, mode, connectString, connectStringLength,
			createParams, authInfo, error);
	dpiOci__handleFree(authInfo, DPI_OCI_HTYPE_AUTHINFO);
	if (status < 0)
		return status;
...
static int dpiConn__getAttributeText(dpiConn *conn, uint32_t attribute,
	...
	case DPI_OCI_ATTR_EDITION:
		...
		DPI_OCI_HTYPE_SESSION, (void*) value, valueLength, attribute,
				"get session value", &error);
		break;
...
int dpiConn__getJsonTDO(dpiConn *conn, dpiError *error) {
	if (conn->jsonTDO)
		return DPI_SUCCESS;
	return dpiOci__typeByName(conn, "SYS", 3, "JSON", 4, &conn->jsonTDO, error);
}
...
int dpiConn__getServerVersion(dpiConn *conn, int wantReleaseString,
	...
	ociCanCache = ((conn->env->versionInfo->versionNum > 20
			|| (conn->env->versionInfo->versionNum == 20
					&& conn->env->versionInfo->releaseNum >= 3))
			&& !wantReleaseString);
	...
	if (releaseString) {
		...
		if (dpiUtils__allocateMemory(1, conn->releaseStringLength, 0,
				"allocate release string", (void**) &conn->releaseString, error)
				< 0)
			return DPI_FAILURE;
	...
	conn->versionInfo.fullVersionNum = (uint32_t) DPI_ORACLE_VERSION_TO_NUMBER(
...
static int dpiConn__setAppContext(void *handle, uint32_t handleType,
	...
	for (i = 0; i < params->numAppContext; i++) {
		...
		if (dpiOci__paramGet(listHandle, DPI_OCI_DTYPE_PARAM, &entryHandle,
				i + 1, "get context entry handle", error) < 0)
			return DPI_FAILURE;
...
static int dpiConn__setAttributesFromCreateParams(dpiConn *conn, void *handle,
	...
	if (userName && userNameLength > 0
			&& dpiOci__attrSet(handle, handleType, (void*) userName,
					userNameLength,
					DPI_OCI_ATTR_USERNAME, "set user name", error) < 0)
		return DPI_FAILURE;
	if (password && passwordLength > 0
			&& dpiOci__attrSet(handle, handleType, (void*) password,
					passwordLength,
...
static int dpiConn__setShardingKey(dpiConn *conn, void **shardingKey,
	...
	if (dpiUtils__checkClientVersion(conn->env->versionInfo, 12, 2, error) < 0)
		return DPI_FAILURE;
	...
	for (i = 0; i < numColumns; i++) {
		if (dpiConn__setShardingKeyValue(conn, *shardingKey, &columns[i], error)
				< 0)
			return DPI_FAILURE;
	}
...
static int dpiConn__setShardingKeyValue(dpiConn *conn, void *shardingKey,
	...
	case DPI_ORACLE_TYPE_NUMBER:
		...
		if (column->nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE) {
			if (dpiDataBuffer__toOracleNumberFromDouble(&column->value, error,
					&numberValue) < 0)
				return DPI_FAILURE;
			convertOk = 1;
		} else if (column->nativeTypeNum == DPI_NATIVE_TYPE_INT64) {
			if (dpiDataBuffer__toOracleNumberFromInteger(&column->value, error,
					&numberValue) < 0)
				return DPI_FAILURE;
			convertOk = 1;
		} else if (column->nativeTypeNum == DPI_NATIVE_TYPE_UINT64) {
			if (dpiDataBuffer__toOracleNumberFromUnsignedInteger(&column->value,
					error, &numberValue) < 0)
				return DPI_FAILURE;
			convertOk = 1;
		} else if (column->nativeTypeNum == DPI_NATIVE_TYPE_BYTES) {
			if (dpiDataBuffer__toOracleNumberFromText(&column->value, conn->env,
					error, &numberValue) < 0)
				return DPI_FAILURE;
			convertOk = 1;
		}
	...
	case DPI_ORACLE_TYPE_DATE:
		if (column->nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP) {
			if (dpiDataBuffer__toOracleDate(&column->value, &dateValue) < 0)
				return DPI_FAILURE;
			convertOk = 1;
		} else if (column->nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE) {
			if (dpiDataBuffer__toOracleDateFromDouble(&column->value, conn->env,
					error, &dateValue) < 0)
				return DPI_FAILURE;
			convertOk = 1;
		}
		...
		if (convertOk) {
			...
			shardingDateValue.century = ((uint8_t) (dateValue.year / 100))
					+ 100;
	...
	case DPI_ORACLE_TYPE_TIMESTAMP_LTZ:
		...
		if (column->nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP) {
			...
			DPI_ORACLE_TYPE_TIMESTAMP_LTZ, conn->env, error, col) < 0) {
				dpiOci__descriptorFree(col, descType);
				return DPI_FAILURE;
			}
			convertOk = 1;
...
static int dpiConn__setXid(dpiConn *conn, dpiXid *xid, dpiError *error) {
	...
	if (dpiOci__attrSet(conn->transactionHandle, DPI_OCI_HTYPE_TRANS, &ociXid,
...
static int dpiConn__startupDatabase(dpiConn *conn, const char *pfile,
	...
	if (pfileLength > 0) {
		...
		DPI_OCI_HTYPE_ADMIN, "create admin handle", error) < 0)
			return DPI_FAILURE;
		if (dpiOci__attrSet(adminHandle, DPI_OCI_HTYPE_ADMIN, (void*) pfile,
				pfileLength, DPI_OCI_ATTR_ADMIN_PFILE, "associate PFILE", error)
				< 0) {
...
int dpiConn_changePassword(dpiConn *conn, const char *userName,
	...
	status = dpiOci__passwordChange(conn, userName, userNameLength, oldPassword,
			oldPasswordLength, newPassword, newPasswordLength,
...
int dpiConn_close(dpiConn *conn, dpiConnCloseMode mode, const char *tag,
	...
	// conditions!
	if (dpiConn__close(conn, mode, tag, tagLength, propagateErrors, &error)
			< 0) {
		if (conn->env->threaded)
			dpiMutex__acquire(conn->env->mutex);
		conn->closing = 0;
...
int dpiConn_create(const dpiContext *context, const char *userName,
	...
	// validate parameters
	if (dpiGen__startPublicFn(context, DPI_HTYPE_CONTEXT, __func__, &error) < 0)
	...
	if (createParams->connectionClass && createParams->connectionClassLength > 0
	...
	if (dpiGen__allocate(DPI_HTYPE_CONN, NULL, (void**) &tempConn, &error) < 0)
		return dpiGen__endPublicFn(context, DPI_FAILURE, &error);
	if (dpiConn__create(tempConn, context, userName, userNameLength, password,
			passwordLength, connectString, connectStringLength,
...
int dpiConn_getSodaDb(dpiConn *conn, dpiSodaDb **db) {
	...
	if (dpiConn__check(conn, __func__, &error) < 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	if (dpiUtils__checkClientVersion(conn->env->versionInfo, 18, 3, &error) < 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	if (dpiUtils__checkDatabaseVersion(conn, 18, 0, &error) < 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	if (dpiGen__allocate(DPI_HTYPE_SODA_DB, conn->env, (void**) db, &error) < 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
...
int dpiConn_deqObject(dpiConn *conn, const char *queueName,
	...
	if (dpiGen__checkHandle(props, DPI_HTYPE_MSG_PROPS,
		...
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	if (dpiGen__checkHandle(payload, DPI_HTYPE_OBJECT, "verify payload", &error)
			< 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
...
int dpiConn_getCallTimeout(dpiConn *conn, uint32_t *value) {
	...
	if (dpiConn__check(conn, __func__, &error) < 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	DPI_CHECK_PTR_NOT_NULL(conn, value)
	if (dpiUtils__checkClientVersion(conn->env->versionInfo, 18, 1, &error) < 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);

	// get call timeout
	status = dpiOci__attrGet(conn->handle, DPI_OCI_HTYPE_SVCCTX, (void*) value,
			0, DPI_OCI_ATTR_CALL_TIMEOUT, "get call timeout", &error);
	return dpiGen__endPublicFn(conn, status, &error);
}
...
//-----------------------------------------------------------------------------
int dpiConn_getEdition(dpiConn *conn, const char **value, uint32_t *valueLength) {
...
int dpiConn_getObjectType(dpiConn *conn, const char *name, uint32_t nameLength,
	...
	if (useTypeByFullName) {
		...
		if (dpiOci__describeAny(conn, tdo, 0, DPI_OCI_OTYPE_PTR, describeHandle,
				&error) < 0) {
			dpiOci__handleFree(describeHandle, DPI_OCI_HTYPE_DESCRIBE);
			return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
		}
	...
	DPI_OCI_HTYPE_DESCRIBE, &param, 0, DPI_OCI_ATTR_PARAM, "get param", &error)
			< 0) {
		dpiOci__handleFree(describeHandle, DPI_OCI_HTYPE_DESCRIBE);
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	}
...
//-----------------------------------------------------------------------------
int dpiConn_getOciAttr(dpiConn *conn, uint32_t handleType, uint32_t attribute,
...
int dpiConn_newJsonQueue(dpiConn *conn, const char *name, uint32_t nameLength,
	...
	status = dpiQueue__allocate(conn, name, nameLength, NULL, queue, 1, &error);
	return dpiGen__endPublicFn(conn, status, &error);
}
...
int dpiConn_newTempLob(dpiConn *conn, dpiOracleTypeNum lobType, dpiLob **lob) {
	...
	case DPI_ORACLE_TYPE_NCLOB:
		...
		break;
	default:
		dpiError__set(&error, "check lob type", DPI_ERR_INVALID_ORACLE_TYPE,
				lobType);
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	}
...
int dpiConn_newQueue(dpiConn *conn, const char *name, uint32_t nameLength,
	...
	DPI_CHECK_PTR_AND_LENGTH(conn, name)
	DPI_CHECK_PTR_NOT_NULL(conn, queue)
	status = dpiQueue__allocate(conn, name, nameLength, payloadType, queue, 0,
			&error);
	return dpiGen__endPublicFn(conn, status, &error);
}
...
int dpiConn_prepareStmt(dpiConn *conn, int scrollable, const char *sql,
		uint32_t sqlLength, const char *tag, uint32_t tagLength, dpiStmt **stmt) {
	dpiStmt *tempStmt;
	dpiError error;
	...
	*stmt = NULL;
	...
	if (dpiStmt__allocate(conn, scrollable, &tempStmt, &error) < 0)
		return dpiGen__endPublicFn(conn, DPI_FAILURE, &error);
	if (dpiStmt__prepare(tempStmt, sql, sqlLength, tag, tagLength, &error)
			< 0) {
...
int dpiConn_setCallTimeout(dpiConn *conn, uint32_t value) {
	...
	// set call timeout
	status = dpiOci__attrSet(conn->handle, DPI_OCI_HTYPE_SVCCTX, &value, 0,
			DPI_OCI_ATTR_CALL_TIMEOUT, "set call timeout", &error);
	return dpiGen__endPublicFn(conn, status, &error);
}
...
// not currently exposed by ODPI-C and should only be used for that purpose.
//-----------------------------------------------------------------------------
int dpiConn_setOciAttr(dpiConn *conn, uint32_t handleType, uint32_t attribute,
	...
	// set attribute value
	status = dpiOci__attrSet(handle, handleType, value, valueLength, attribute,
			"generic set OCI attribute", &error);
	return dpiGen__endPublicFn(conn, status, &error);
}
...
int dpiConn_getTransactionInProgress(dpiConn *conn, int *value) {
	...
	status = dpiOci__attrGet(conn->sessionHandle, DPI_OCI_HTYPE_SESSION, &temp,
			NULL, DPI_OCI_ATTR_TRANSACTION_IN_PROGRESS,
