...
static int dpiSodaDb__createDocument(dpiSodaDb *db, const char *key,
	...
	if (key && keyLength > 0) {
		if (dpiOci__attrSet(tempDoc->handle, DPI_OCI_HTYPE_SODA_DOCUMENT,
				(void*) key, keyLength, DPI_OCI_ATTR_SODA_KEY, "set key", error)
				< 0) {
...
// the collection.
//-----------------------------------------------------------------------------
int dpiSodaDb_createDocument(dpiSodaDb *db, const char *key, uint32_t keyLength,
		const char *content, uint32_t contentLength, const char *mediaType,
		uint32_t mediaTypeLength, UNUSED uint32_t flags, dpiSodaDoc **doc) {
	dpiError error;
	int status;
...
int dpiSodaDb_createJsonDocument(dpiSodaDb *db, const char *key,
	...
	if (content) {
		...
		if (status == DPI_SUCCESS) {
			...
			DPI_OCI_HTYPE_SODA_DOCUMENT, (void*) &jsonHandle, &tempLength,
					DPI_OCI_ATTR_SODA_CONTENT, "get JSON descriptor", &error);
		}
		if (status == DPI_SUCCESS)
...
int dpiSodaDb_openCollection(dpiSodaDb *db, const char *name,
	...
	if (flags & DPI_SODA_FLAGS_ATOMIC_COMMIT)
		mode |= DPI_OCI_SODA_ATOMIC_COMMIT;
	if (dpiOci__sodaCollOpen(db, name, nameLength, mode, &handle, &error) < 0)
		return dpiGen__endPublicFn(db, DPI_FAILURE, &error);
	*coll = NULL;
	if (handle) {
		if (dpiSodaColl__allocate(db, handle, coll, &error) < 0) {
