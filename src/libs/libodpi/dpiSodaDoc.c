...
int dpiSodaDoc__allocate(dpiSodaDb *db, void *handle, dpiSodaDoc **doc,
	...
	if (dpiGen__allocate(DPI_HTYPE_SODA_DOC, db->env, (void**) &tempDoc, error)
			< 0) {
		if (handle)
	...
	if (!handle) {
		...
		DPI_OCI_HTYPE_SODA_DOCUMENT, "allocate SODA document handle", error);
...
int dpiSodaDoc_getContent(dpiSodaDoc *doc, const char **value,
	...
	// get content
	if (dpiOci__attrGet(doc->handle, DPI_OCI_HTYPE_SODA_DOCUMENT, (void*) value,
			valueLength, DPI_OCI_ATTR_SODA_CONTENT, "get content", &error) < 0)
		return dpiGen__endPublicFn(doc, DPI_FAILURE, &error);
	...
	// otherwise, determine the encoding from OCI
		...
		case DPI_CHARSET_ID_UTF16LE:
			*encoding = DPI_CHARSET_NAME_UTF16LE;
			break;
		default:
			dpiError__set(&error, "check charset", DPI_ERR_INVALID_CHARSET_ID,
					charsetId);
			return dpiGen__endPublicFn(doc, DPI_FAILURE, &error);
		}
	}
...
int dpiSodaDoc_getVersion(dpiSodaDoc *doc, const char **value,
		uint32_t *valueLength) {
	return dpiSodaDoc__getAttributeText(doc, DPI_OCI_ATTR_SODA_VERSION, value,
			valueLength, __func__);
}
