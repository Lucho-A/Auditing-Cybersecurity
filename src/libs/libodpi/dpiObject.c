...
//-----------------------------------------------------------------------------
int dpiObject__allocate(dpiObjectType *objType, void *instance, void *indicator,
		dpiObject *dependsOnObj, dpiObject **obj, dpiError *error) {
	dpiObject *tempObj;
...
//-----------------------------------------------------------------------------
static int dpiObject__check(dpiObject *obj, const char *fnName, dpiError *error) {
...
static void dpiObject__clearOracleValue(dpiObject *obj, dpiError *error,
	...
	case DPI_ORACLE_TYPE_NVARCHAR:
		if (buffer->asString)
			dpiOci__stringResize(obj->env->handle, &buffer->asString, 0, error);
		break;
	case DPI_ORACLE_TYPE_RAW:
		if (buffer->asRawData)
			dpiOci__rawResize(obj->env->handle, &buffer->asRawData, 0, error);
		break;
...
//-----------------------------------------------------------------------------
int dpiObject__closeHelper(dpiObject *obj, int checkError, dpiError *error) {
	if (dpiOci__objectFree(obj->env->handle, obj->instance, checkError, error)
			< 0)
		return DPI_FAILURE;
	obj->instance = NULL;
	if (obj->freeIndicator
			&& dpiOci__objectFree(obj->env->handle, obj->indicator, checkError,
					error) < 0)
		return DPI_FAILURE;
	obj->indicator = NULL;
	if (!obj->type->conn->closing)
		dpiHandleList__removeHandle(obj->type->conn->objects, obj->openSlotNum);
	return DPI_SUCCESS;
}
...
static int dpiObject__fromOracleValue(dpiObject *obj, dpiError *error,
	...
	case DPI_ORACLE_TYPE_NUMBER:
		if (nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE)
			...
			return dpiDataBuffer__fromOracleNumberAsInteger(&data->value, error,
					value->asNumber);
			...
			return dpiDataBuffer__fromOracleNumberAsText(&data->value, obj->env,
					error, value->asNumber);
		break;
	case DPI_ORACLE_TYPE_DATE:
		if (nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP)
			return dpiDataBuffer__fromOracleDate(&data->value, value->asDate);
		if (nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE)
			return dpiDataBuffer__fromOracleDateAsDouble(&data->value, obj->env,
					error, value->asDate);
		break;
	case DPI_ORACLE_TYPE_TIMESTAMP:
		if (nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP)
			return dpiDataBuffer__fromOracleTimestamp(&data->value, obj->env,
					error, *value->asTimestamp, 0);
		if (nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE)
			return dpiDataBuffer__fromOracleTimestampAsDouble(&data->value,
					valueOracleTypeNum, obj->env, error, *value->asTimestamp);
		break;
	case DPI_ORACLE_TYPE_TIMESTAMP_TZ:
	case DPI_ORACLE_TYPE_TIMESTAMP_LTZ:
		if (nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP)
			return dpiDataBuffer__fromOracleTimestamp(&data->value, obj->env,
					error, *value->asTimestamp, 1);
		if (nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE)
			return dpiDataBuffer__fromOracleTimestampAsDouble(&data->value,
					valueOracleTypeNum, obj->env, error, *value->asTimestamp);
		break;
	case DPI_ORACLE_TYPE_OBJECT:
		if (typeInfo->objectType && nativeTypeNum == DPI_NATIVE_TYPE_OBJECT) {
			void *instance =
					(typeInfo->objectType->isCollection) ?
							*value->asCollection : value->asRaw;
			dpiObject *tempObj;
			if (dpiObject__allocate(typeInfo->objectType, instance, indicator,
					obj, &tempObj, error) < 0)
				return DPI_FAILURE;
	...
	case DPI_ORACLE_TYPE_BFILE:
		if (nativeTypeNum == DPI_NATIVE_TYPE_LOB) {
			...
			void *tempLocator;
			dpiLob *tempLob;
			lobType = dpiOracleType__getFromNum(typeInfo->oracleTypeNum, error);
			if (dpiLob__allocate(obj->type->conn, lobType, &tempLob, error) < 0)
				return DPI_FAILURE;
			tempLocator = tempLob->locator;
			tempLob->locator = *(value->asLobLocator);
			if (dpiOci__lobLocatorAssign(tempLob, &tempLocator, error) < 0) {
	...
	*valueIndicator = DPI_OCI_IND_NOTNULL;
	...
	case DPI_ORACLE_TYPE_NUMBER:
		*ociValue = &buffer->asNumber;
		if (nativeTypeNum == DPI_NATIVE_TYPE_INT64)
			return dpiDataBuffer__toOracleNumberFromInteger(&data->value, error,
					&buffer->asNumber);
		if (nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE)
			return dpiDataBuffer__toOracleNumberFromDouble(&data->value, error,
					&buffer->asNumber);
		if (nativeTypeNum == DPI_NATIVE_TYPE_BYTES)
			return dpiDataBuffer__toOracleNumberFromText(&data->value, obj->env,
	...
	case DPI_ORACLE_TYPE_DATE:
		*ociValue = &buffer->asDate;
		if (nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP)
			return dpiDataBuffer__toOracleDate(&data->value, &buffer->asDate);
		if (nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE)
			return dpiDataBuffer__toOracleDateFromDouble(&data->value, obj->env,
					error, &buffer->asDate);
		break;
	case DPI_ORACLE_TYPE_TIMESTAMP:
	case DPI_ORACLE_TYPE_TIMESTAMP_TZ:
	case DPI_ORACLE_TYPE_TIMESTAMP_LTZ:
		...
		if (nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP
			...
			if (dpiOci__descriptorAlloc(obj->env->handle, &buffer->asTimestamp,
			...
			*ociValue = buffer->asTimestamp;
			...
			return dpiDataBuffer__toOracleTimestampFromDouble(&data->value,
					valueOracleTypeNum, obj->env, error, buffer->asTimestamp);
		}
		break;
	case DPI_ORACLE_TYPE_OBJECT:
		...
		if (nativeTypeNum == DPI_NATIVE_TYPE_OBJECT) {
			if (otherObj->type->tdo != dataTypeInfo->objectType->tdo)
				return dpiError__set(error, "check type", DPI_ERR_WRONG_TYPE,
						otherObj->type->schemaLength, otherObj->type->schema,
						otherObj->type->nameLength, otherObj->type->name,
		...
		if (nativeTypeNum == DPI_NATIVE_TYPE_LOB) {
			*ociValue = data->value.asLOB->locator;
			...
			if (dpiLob__allocate(obj->type->conn, lobType, lob, error) < 0)
				return DPI_FAILURE;
			bytes = &data->value.asBytes;
			if (dpiLob__setFromBytes(*lob, bytes->ptr, bytes->length, error)
					< 0)
				return DPI_FAILURE;
	...
	return dpiError__set(error, "to Oracle value", DPI_ERR_UNHANDLED_CONVERSION,
			valueOracleTypeNum, nativeTypeNum);
}
...
int dpiObject_copy(dpiObject *obj, dpiObject **copiedObj) {
	...
	if (dpiObject__allocate(obj->type, NULL, NULL, NULL, &tempObj, &error) < 0)
		return dpiGen__endPublicFn(obj, DPI_FAILURE, &error);
	if (dpiOci__objectCopy(tempObj, obj->instance, obj->indicator, &error)
			< 0) {
		dpiObject__free(tempObj, &error);
		return dpiGen__endPublicFn(obj, DPI_FAILURE, &error);
	}
...
int dpiObject_getAttributeValue(dpiObject *obj, dpiObjectAttr *attr,
	...
	// get attribute value
	if (dpiOci__objectGetAttr(obj, attr, &scalarValueIndicator, &valueIndicator,
			&value.asRaw, &tdo, &error) < 0)
		return dpiGen__endPublicFn(obj, DPI_FAILURE, &error);
