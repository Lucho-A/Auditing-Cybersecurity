...
int dpiPool__acquireConnection(dpiPool *pool, const char *userName,
	...
	if (dpiGen__allocate(DPI_HTYPE_CONN, pool->env, (void**) &tempConn, error)
			< 0)
		return DPI_FAILURE;
	error->env = pool->env;
...
static int dpiPool__create(dpiPool *pool, const char *userName,
	...
	if (commonParams->accessToken) {
		...
		if (createParams->accessTokenCallback) {
			...
			if (dpiOci__attrSet(authInfo, DPI_OCI_HTYPE_SESSION, (void*) pool,
					0, DPI_OCI_ATTR_TOKEN_CBKCTX, "set token callback context",
					error) < 0)
				return DPI_FAILURE;
	...
	if (createParams->plsqlFixupCallback
		...
		if (dpiUtils__checkClientVersion(pool->env->versionInfo, 12, 2, error)
				< 0)
			return DPI_FAILURE;
	...
	if (dpiOci__attrSet(pool->handle, DPI_OCI_HTYPE_SPOOL,
			(void*) &createParams->timeout, 0, DPI_OCI_ATTR_SPOOL_TIMEOUT,
			"set timeout", error) < 0)
		return DPI_FAILURE;
	...
	if (pool->env->versionInfo->versionNum > 12
		...
		if (dpiOci__attrSet(pool->handle, DPI_OCI_HTYPE_SPOOL,
				(void*) &createParams->waitTimeout, 0,
				DPI_OCI_ATTR_SPOOL_WAIT_TIMEOUT, "set wait timeout", error) < 0)
			return DPI_FAILURE;
	}
	...
	if (pool->env->versionInfo->versionNum >= 12) {
		if (dpiOci__attrSet(pool->handle, DPI_OCI_HTYPE_SPOOL,
				(void*) &createParams->maxLifetimeSession, 0,
	...
	if (pool->env->versionInfo->versionNum > 18
		...
		if (dpiOci__attrSet(pool->handle, DPI_OCI_HTYPE_SPOOL,
				(void*) &createParams->maxSessionsPerShard, 0,
				DPI_OCI_ATTR_SPOOL_MAX_PER_SHARD, "set max sessions per shard",
				error) < 0)
			return DPI_FAILURE;
	}
	...
	if (dpiOci__attrSet(pool->handle, DPI_OCI_HTYPE_SPOOL,
			...
			DPI_OCI_ATTR_SPOOL_STMTCACHESIZE, "set stmt cache size", error) < 0)
		return DPI_FAILURE;
...
static int dpiPool__getAttributeUint(dpiPool *pool, uint32_t attribute,
	...
	case DPI_OCI_ATTR_SPOOL_MAX_LIFETIME_SESSION:
		if (dpiUtils__checkClientVersion(pool->env->versionInfo, 12, 1, &error)
				< 0)
			return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
		break;
	...
	case DPI_OCI_ATTR_SPOOL_MAX_PER_SHARD:
		if (dpiUtils__checkClientVersion(pool->env->versionInfo, 18, 3, &error)
				< 0)
			return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
		break;
	default:
		dpiError__set(&error, "get attribute value", DPI_ERR_NOT_SUPPORTED);
		return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
	}
...
static int dpiPool__setAttributeUint(dpiPool *pool, uint32_t attribute,
	...
	case DPI_OCI_ATTR_SPOOL_MAX_PER_SHARD:
		...
		break;
	default:
		dpiError__set(&error, "set attribute value", DPI_ERR_NOT_SUPPORTED);
		return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
	}
...
int dpiPool_getBusyCount(dpiPool *pool, uint32_t *value) {
	return dpiPool__getAttributeUint(pool, DPI_OCI_ATTR_SPOOL_BUSY_COUNT, value,
			__func__);
}
...
int dpiPool_getOpenCount(dpiPool *pool, uint32_t *value) {
	return dpiPool__getAttributeUint(pool, DPI_OCI_ATTR_SPOOL_OPEN_COUNT, value,
			__func__);
}
...
int dpiPool_getSodaMetadataCache(dpiPool *pool, int *enabled) {
	...
	if (dpiUtils__checkClientVersionMulti(pool->env->versionInfo, 19, 11, 21, 3,
			&error) < 0)
		return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
	if (dpiOci__attrGet(pool->env->handle, DPI_OCI_HTYPE_ENV, enabled, NULL,
	DPI_OCI_ATTR_SODA_METADATA_CACHE, "get SODA metadata cache", &error) < 0)
		return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
...
int dpiPool_setAccessToken(dpiPool *pool, dpiAccessToken *accessToken) {
	...
	NULL, DPI_OCI_ATTR_SPOOL_AUTH, "get attribute value", &error) < 0)
		return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
...
int dpiPool_setSodaMetadataCache(dpiPool *pool, int enabled) {
	...
	DPI_OCI_ATTR_SODA_METADATA_CACHE, "set SODA metadata cache", &error) < 0)
		return dpiGen__endPublicFn(pool, DPI_FAILURE, &error);
