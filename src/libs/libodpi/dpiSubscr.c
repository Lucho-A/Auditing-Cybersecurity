...
int dpiSubscr__create(dpiSubscr *subscr, dpiConn *conn,
	...
	if (qosFlags
			&& dpiOci__attrSet(subscr->handle, DPI_OCI_HTYPE_SUBSCRIPTION,
					...
					DPI_OCI_ATTR_SUBSCR_QOSFLAGS, "set QOS", error) < 0)
		return DPI_FAILURE;
	...
	if (params->qos & DPI_SUBSCR_QOS_ROWIDS) {
		rowids = 1;
		if (dpiOci__attrSet(subscr->handle, DPI_OCI_HTYPE_SUBSCRIPTION,
				(void*) &rowids, 0, DPI_OCI_ATTR_CHNF_ROWIDS, "set rowids flag",
				error) < 0)
			return DPI_FAILURE;
	}
	...
	if (params->clientInitiated) {
		if (dpiUtils__checkClientVersion(conn->env->versionInfo, 19, 4, error)
				< 0)
			return DPI_FAILURE;
...
static int dpiSubscr__populateMessage(dpiSubscr *subscr,
	...
	if (subscr->subscrNamespace == DPI_SUBSCR_NAMESPACE_AQ)
		return dpiSubscr__populateAQMessage(subscr, message, descriptor, error);
...
static int dpiSubscr__populateMessageQuery(dpiSubscr *subscr,
	...
	DPI_OCI_ATTR_CQDES_TABLE_CHANGES, "get table descriptor", error) < 0)
		return DPI_FAILURE;
	if (!tables)
		return DPI_SUCCESS;
...
static int dpiSubscr__populateMessageRow(dpiSubscrMessageRow *row,
	...
	// determine operation
	if (dpiOci__attrGet(descriptor, DPI_OCI_DTYPE_ROW_CHDES, &row->operation, 0,
			DPI_OCI_ATTR_CHDES_ROW_OPFLAGS, "get operation", error) < 0)
		return DPI_FAILURE;
...
static int dpiSubscr__populateMessageTable(dpiSubscr *subscr,
	...
	DPI_OCI_ATTR_CHDES_TABLE_ROW_CHANGES, "get rows descriptor", error) < 0)
		return DPI_FAILURE;
...
int dpiSubscr_prepareStmt(dpiSubscr *subscr, const char *sql,
	...
	if (dpiStmt__allocate(subscr->conn, 0, &tempStmt, &error) < 0)
		return dpiGen__endPublicFn(subscr, DPI_FAILURE, &error);
	if (dpiSubscr__prepareStmt(subscr, tempStmt, sql, sqlLength, &error) < 0) {
