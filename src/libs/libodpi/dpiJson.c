...
int dpiJson__allocate(dpiConn *conn, void *handle, dpiJson **json,
	...
	if (dpiUtils__checkClientVersion(conn->env->versionInfo, 21, 0, error) < 0)
		return DPI_FAILURE;
	if (dpiGen__allocate(DPI_HTYPE_JSON, conn->env, (void**) &tempJson, error)
			< 0)
		return DPI_FAILURE;
...
static int dpiJsonNode__fromOracleArrayToNative(dpiJson *json,
	...
	array->numElements = (*domDoc->methods->fnGetArraySize)(domDoc, oracleNode);
	...
	while (pos < array->numElements) {
		numInBatch = (*domDoc->methods->fnGetArrayElemBatch)(domDoc, oracleNode,
				pos, DPI_JSON_BATCH_NODES, oracleElementNodes);
...
static int dpiJsonNode__fromOracleObjectToNative(dpiJson *json,
	...
	if (dpiUtils__allocateMemory(obj->numFields, sizeof(dpiJsonNode), 1,
			"allocate JSON object field nodes", (void**) &obj->fields, error)
			< 0)
		return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
static int dpiJsonNode__fromOracleNumberAsText(dpiJson *json, dpiJsonNode *node,
		uint8_t *numBuffer, dpiError *error) {
	char **tempBuffers;
	...
	return dpiDataBuffer__fromOracleNumberAsText(node->value, json->env, error,
			numBuffer);
}
...
static int dpiJsonNode__fromOracleScalarToNative(dpiJson *json,
	...
	case DPI_JZNVAL_ORA_TIMESTAMPTZ:
		node->oracleTypeNum =
				(scalar.valueType == DPI_JZNVAL_ORA_DATE) ?
						DPI_ORACLE_TYPE_DATE : DPI_ORACLE_TYPE_TIMESTAMP;
		if (options & DPI_JSON_OPT_DATE_AS_DOUBLE) {
			node->nativeTypeNum = DPI_NATIVE_TYPE_DOUBLE;
			if (dpiDataBuffer__fromOracleDateAsDouble(node->value, json->env,
					error, (dpiOciDate*) &ociVal.asJsonDateTime) < 0)
				return DPI_FAILURE;
			node->value->asDouble += (ociVal.asJsonDateTime.fsecond / 1000000);
			return DPI_SUCCESS;
		}
	...
	case DPI_JZNVAL_ORA_DAYSECOND_DUR:
		...
		node->value->asIntervalDS.minutes = ociVal.asJsonDayInterval.minutes;
		node->value->asIntervalDS.seconds = ociVal.asJsonDayInterval.seconds;
		node->value->asIntervalDS.fseconds = ociVal.asJsonDayInterval.fseconds;
		return DPI_SUCCESS;
	case DPI_JZNVAL_ORA_YEARMONTH_DUR:
		...
		node->value->asIntervalYM.months = ociVal.asJsonYearInterval.months;
		return DPI_SUCCESS;
...
static int dpiJsonNode__toOracleFromNative(dpiJson *json,
	...
	case DPI_ORACLE_TYPE_NUMBER:
		if (node->nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE) {
			if (dpiDataBuffer__toOracleNumberFromDouble(node->value, error,
					&dataBuffer.asNumber) < 0)
				return DPI_FAILURE;
		} else if (node->nativeTypeNum == DPI_NATIVE_TYPE_INT64) {
			if (dpiDataBuffer__toOracleNumberFromInteger(node->value, error,
					&dataBuffer.asNumber) < 0)
				return DPI_FAILURE;
		} else if (node->nativeTypeNum == DPI_NATIVE_TYPE_UINT64) {
			if (dpiDataBuffer__toOracleNumberFromUnsignedInteger(node->value,
					error, &dataBuffer.asNumber) < 0)
				return DPI_FAILURE;
		} else if (node->nativeTypeNum == DPI_NATIVE_TYPE_BYTES) {
			if (dpiDataBuffer__toOracleNumberFromText(node->value, json->env,
					error, &dataBuffer.asNumber) < 0)
				return DPI_FAILURE;
		} else {
			break;
	...
	case DPI_ORACLE_TYPE_DATE:
		if (node->nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP) {
			if (dpiDataBuffer__toOracleDate(node->value, &dataBuffer.asDate)
					< 0)
				return DPI_FAILURE;
		} else if (node->nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE) {
			if (dpiDataBuffer__toOracleDateFromDouble(node->value, json->env,
					error, &dataBuffer.asDate) < 0)
				return DPI_FAILURE;
		} else {
			break;
		}
	...
	case DPI_ORACLE_TYPE_TIMESTAMP:
		if (!json->convTimestamp) {
			if (dpiOci__descriptorAlloc(json->env->handle, &json->convTimestamp,
					DPI_OCI_DTYPE_TIMESTAMP, "alloc timestamp for JSON", error)
					< 0)
				return DPI_FAILURE;
		}
		if (node->nativeTypeNum == DPI_NATIVE_TYPE_TIMESTAMP) {
			if (dpiDataBuffer__toOracleTimestamp(node->value, json->env, error,
					json->convTimestamp, 0) < 0)
				return DPI_FAILURE;
		} else if (node->nativeTypeNum == DPI_NATIVE_TYPE_DOUBLE) {
			if (dpiDataBuffer__toOracleTimestampFromDouble(node->value,
					node->oracleTypeNum, json->env, error, json->convTimestamp)
					< 0)
				return DPI_FAILURE;
		} else {
			break;
	...
	case DPI_ORACLE_TYPE_INTERVAL_DS:
		...
		if (node->nativeTypeNum == DPI_NATIVE_TYPE_INTERVAL_DS) {
			if (dpiDataBuffer__toOracleIntervalDS(node->value, json->env, error,
					json->convIntervalDS) < 0)
				return DPI_FAILURE;
			*oracleNode = domDoc->methods->fnNewScalarVal(domDoc,
	...
	case DPI_ORACLE_TYPE_INTERVAL_YM:
		...
		if (node->nativeTypeNum == DPI_NATIVE_TYPE_INTERVAL_YM) {
			if (dpiDataBuffer__toOracleIntervalYM(node->value, json->env, error,
					json->convIntervalYM) < 0)
				return DPI_FAILURE;
			*oracleNode = domDoc->methods->fnNewScalarVal(domDoc,
	...
	case DPI_ORACLE_TYPE_BOOLEAN:
		if (node->nativeTypeNum == DPI_NATIVE_TYPE_BOOLEAN) {
			...
			DPI_JZNVAL_TRUE :
													DPI_JZNVAL_FALSE;
			*oracleNode = domDoc->methods->fnNewScalarVal(domDoc, scalarType,
					NULL);
			return DPI_SUCCESS;
		}
		break;
...
int dpiJson__setValue(dpiJson *json, const dpiJsonNode *topNode,
	...
	if (dpiOci__attrSet(json->handle, DPI_OCI_DTYPE_JSON, (void*) &mutable, 0,
			DPI_OCI_ATTR_JSON_DOM_MUTABLE, "set JSON descriptor mutable", error)
			< 0)
		return DPI_FAILURE;
...
int dpiJson_setFromText(dpiJson *json, const char *value, uint64_t valueLength,
	...
	status = dpiOci__jsonTextBufferParse(json, value, valueLength, flags,
			&error);
