...
static int dpiDeqOptions__getAttrValue(dpiDeqOptions *options,
	...
	if (dpiGen__startPublicFn(options, DPI_HTYPE_DEQ_OPTIONS, fnName, &error)
			< 0)
...
int dpiDeqOptions_getMsgId(dpiDeqOptions *options, const char **value,
	...
	if (dpiGen__startPublicFn(options, DPI_HTYPE_DEQ_OPTIONS, __func__, &error)
			< 0)
		return dpiGen__endPublicFn(options, DPI_FAILURE, &error);
	DPI_CHECK_PTR_NOT_NULL(options, value)
	DPI_CHECK_PTR_NOT_NULL(options, valueLength)
	if (dpiOci__attrGet(options->handle, DPI_OCI_DTYPE_AQDEQ_OPTIONS, &rawValue,
			NULL, DPI_OCI_ATTR_DEQ_MSGID, "get attribute value", &error) < 0)
		return dpiGen__endPublicFn(options, DPI_FAILURE, &error);
...
int dpiDeqOptions_setMode(dpiDeqOptions *options, dpiDeqMode value) {
	return dpiDeqOptions__setAttrValue(options, DPI_OCI_ATTR_DEQ_MODE, __func__,
			&value, 0);
}
