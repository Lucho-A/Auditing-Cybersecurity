...
static int dpiStmt__reExecute(dpiStmt *stmt, uint32_t numIters, uint32_t mode,
		dpiError *error);
...
int dpiStmt__allocate(dpiConn *conn, int scrollable, dpiStmt **stmt,
	...
	*stmt = NULL;
	if (dpiGen__allocate(DPI_HTYPE_STMT, conn->env, (void**) &tempStmt, error)
			< 0)
		return DPI_FAILURE;
...
static int dpiStmt__bind(dpiStmt *stmt, dpiVar *var, uint32_t pos,
	...
	if (found) {
		...
		if (name) {
			if (dpiUtils__allocateMemory(1, nameLength, 0,
					"allocate memory for name", (void**) &entry->name, error)
					< 0)
				return DPI_FAILURE;
	...
	if (pos > 0) {
		if (stmt->env->versionInfo->versionNum < 12)
			status = dpiOci__bindByPos(stmt, &bindHandle, pos, dynamicBind, var,
					error);
		else
...
//-----------------------------------------------------------------------------
static int dpiStmt__createBindVar(dpiStmt *stmt, dpiNativeTypeNum nativeTypeNum,
		dpiData *data, dpiVar **var, uint32_t pos, const char *name,
		uint32_t nameLength, dpiError *error) {
...
static int dpiStmt__createQueryVars(dpiStmt *stmt, dpiError *error) {
	...
	if (dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, (void*) &numQueryVars,
			0, DPI_OCI_ATTR_PARAM_COUNT, "get parameter count", error) < 0)
		return DPI_FAILURE;
	...
	if (numQueryVars != stmt->numQueryVars) {
		...
		for (i = 0; i < numQueryVars; i++) {
			if (dpiStmt__getQueryInfo(stmt, i + 1, &stmt->queryInfo[i], error)
					< 0) {
				dpiStmt__clearQueryVars(stmt, error);
				return DPI_FAILURE;
			}
...
static int dpiStmt__define(dpiStmt *stmt, uint32_t pos, dpiVar *var,
	...
	if (var->buffer.objectIndicator
			&& dpiOci__defineObject(var, defineHandle, error) < 0)
		return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
static int dpiStmt__execute(dpiStmt *stmt, uint32_t numIters, uint32_t mode,
		int reExecute, dpiError *error) {
	uint16_t tempOffset;
	uint32_t i, j, temp;
	dpiData *data;
	dpiVar *var;
	...
	if (stmt->statementType == DPI_STMT_TYPE_SELECT) {
		...
		if (dpiOci__attrSet(stmt->handle, DPI_OCI_HTYPE_STMT, &temp,
				sizeof(temp), DPI_OCI_ATTR_PREFETCH_ROWS, "reset prefetch rows",
				error) < 0)
			return DPI_FAILURE;
	}
...
static int dpiStmt__fetch(dpiStmt *stmt, dpiError *error) {
	...
	if (dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, &stmt->bufferRowCount,
			0, DPI_OCI_ATTR_ROWS_FETCHED, "get rows fetched", error) < 0)
		return DPI_FAILURE;
...
static int dpiStmt__getBatchErrors(dpiStmt *stmt, dpiError *error) {
	...
	if (dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, &stmt->numBatchErrors,
			0, DPI_OCI_ATTR_NUM_DML_ERRORS, "get batch error count", error) < 0)
		return DPI_FAILURE;
	...
	DPI_OCI_HTYPE_ERROR, "allocate parameter error handle", error) < 0) {
		dpiStmt__clearBatchErrors(stmt);
		return DPI_FAILURE;
	}
	...
	for (i = 0; i < stmt->numBatchErrors; i++) {
		...
		if (dpiOci__attrGet(batchErrorHandle, DPI_OCI_HTYPE_ERROR, &rowOffset,
				0, DPI_OCI_ATTR_DML_ROW_OFFSET, "get row offset", &localError)
				< 0) {
...
//-----------------------------------------------------------------------------
static int dpiStmt__getRowCount(dpiStmt *stmt, uint64_t *count, dpiError *error) {
	uint32_t rowCount32;
	...
	for (i = 0; i < stmt->numQueryVars; i++) {
		...
		if (!var) {
			...
			if (dpiVar__allocate(stmt->conn, queryInfo->typeInfo.oracleTypeNum,
					...
					stmt->fetchArraySize, queryInfo->typeInfo.clientSizeInBytes,
					1, 0, queryInfo->typeInfo.objectType, &var, &data, error)
					< 0)
				return DPI_FAILURE;
		...
		if (var->requiresPreFetch
				&& dpiVar__extendedPreFetch(var, &var->buffer, error) < 0)
			return DPI_FAILURE;
	}
...
static int dpiStmt__reExecute(dpiStmt *stmt, uint32_t numIters, uint32_t mode,
	...
	if (dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, (void*) &sql,
			&sqlLength, DPI_OCI_ATTR_STATEMENT, "get statement", &localError)
			< 0)
		return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
int dpiStmt_fetchRows(dpiStmt *stmt, uint32_t maxRows, uint32_t *bufferRowIndex,
		uint32_t *numRowsFetched, int *moreRows) {
	dpiError error;
...
int dpiStmt_getBindCount(dpiStmt *stmt, uint32_t *count) {
	...
	if (dpiStmt__check(stmt, __func__, &error) < 0)
		return dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
	DPI_CHECK_PTR_NOT_NULL(stmt, count)
	status = dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, (void*) count, 0,
...
int dpiStmt_getImplicitResult(dpiStmt *stmt, dpiStmt **implicitResult) {
	...
	if (dpiUtils__checkClientVersion(stmt->env->versionInfo, 12, 1, &error) < 0)
		return dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
...
int dpiStmt_getNumQueryColumns(dpiStmt *stmt, uint32_t *numQueryColumns) {
	...
	if (stmt->statementType == DPI_STMT_TYPE_SELECT && stmt->numQueryVars == 0
			&& dpiStmt__createQueryVars(stmt, &error) < 0)
		return dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
	*numQueryColumns = stmt->numQueryVars;
	return dpiGen__endPublicFn(stmt, DPI_SUCCESS, &error);
