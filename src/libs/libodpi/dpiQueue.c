...
static int dpiQueue__getPayloadTDO(dpiQueue *queue, void **tdo, dpiError *error);
...
int dpiQueue__allocate(dpiConn *conn, const char *name, uint32_t nameLength,
	...
	if (dpiGen__allocate(DPI_HTYPE_QUEUE, conn->env, (void**) &tempQueue, error)
			< 0)
		return DPI_FAILURE;
...
static int dpiQueue__allocateBuffer(dpiQueue *queue, uint32_t numElements,
	...
	if (dpiUtils__allocateMemory(numElements, sizeof(dpiMsgProps*), 1,
			"allocate msg props array", (void**) &queue->buffer.props, error)
			< 0)
		return DPI_FAILURE;
	...
	if (dpiUtils__allocateMemory(numElements, sizeof(void*), 1,
		...
		return DPI_FAILURE;
	if (dpiUtils__allocateMemory(numElements, sizeof(void*), 1,
			"allocate OCI indicators array", (void**) &queue->buffer.indicators,
			error) < 0)
		return DPI_FAILURE;
	...
	if (dpiUtils__allocateMemory(numElements, sizeof(void*), 1,
			"allocate message ids array", (void**) &queue->buffer.msgIds, error)
			< 0)
		return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
static int dpiQueue__check(dpiQueue *queue, const char *fnName, dpiError *error) {
...
static int dpiQueue__deq(dpiQueue *queue, uint32_t *numProps,
	...
	for (i = 0; i < *numProps; i++) {
		...
		if (queue->isJson) {
			if (dpiJson__allocate(queue->conn, NULL, &prop->payloadJson, error)
					< 0)
				return DPI_FAILURE;
		}
	...
	if (*numProps == 1) {
		status = dpiOci__aqDeq(queue->conn, queue->name,
				queue->deqOptions->handle, queue->buffer.handles[0], payloadTDO,
		...
		for (i = 0; i < *numProps; i++) {
			status = dpiOci__aqDeq(queue->conn, queue->name,
					...
					&queue->buffer.indicators[i], &queue->buffer.msgIds[i],
					error);
			if (status < 0) {
				*numProps = i;
				break;
...
static int dpiQueue__enq(dpiQueue *queue, uint32_t numProps,
	...
	for (i = 0; i < numProps; i++) {
		...
		if (queue->payloadType && props[i]->payloadObj
			...
			return dpiError__set(error, "check payload", DPI_ERR_WRONG_TYPE,
					...
					queue->payloadType->schema, queue->payloadType->nameLength,
