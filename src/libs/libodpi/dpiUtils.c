...
int dpiUtils__allocateMemory(size_t numMembers, size_t memberSize,
	...
	if (clearMemory)
		...
		*ptr = malloc(numMembers * memberSize);
	if (!*ptr)
...
int dpiUtils__getAttrStringWithDup(const char *action, const void *ociHandle,
	...
	if (dpiOci__attrGet(ociHandle, ociHandleType, (void*) &source, valueLength,
			ociAttribute, action, error) < 0)
		return DPI_FAILURE;
	if (*valueLength == 0) {
		*value = NULL;
	} else {
...
int dpiUtils__parseNumberString(const char *value, uint32_t valueLength,
	...
	*decimalPointIndex = *numDigits;
	...
	// number as well
	if (value < endValue)
		return dpiError__set(error, "check string used", DPI_ERR_INVALID_NUMBER);
...
int dpiUtils__setAttributesFromCommonCreateParams(void *handle,
	...
	if (params->edition && params->editionLength > 0
			&& dpiOci__attrSet(handle, handleType, (void*) params->edition,
					params->editionLength,
...
int dpiUtils__setAccessTokenAttributes(void *handle,
	...
	if (accessToken->privateKey) {
		if (dpiUtils__checkClientVersionMulti(versionInfo, 19, 14, 21, 5, error)
				< 0)
			return DPI_FAILURE;

		// OAuth feature only available in Oracle Client 19.15+ and 21.7+ libraries
	} else {
		if (dpiUtils__checkClientVersionMulti(versionInfo, 19, 15, 21, 7, error)
				< 0)
			return DPI_FAILURE;
	}
	...
	if (accessToken->privateKey) {
		if (dpiOci__attrSet(handle, DPI_OCI_HTYPE_AUTHINFO,
				...
				DPI_OCI_ATTR_IAM_PRIVKEY, "set access token private key", error)
				< 0)
			return DPI_FAILURE;

		// set OAuth bearer flag on auth handle
	} else {
		if (dpiOci__attrSet(handle, DPI_OCI_HTYPE_AUTHINFO, (void*) &isBearer,
				0, DPI_OCI_ATTR_TOKEN_ISBEARER, "set bearer flag", error) < 0)
			return DPI_FAILURE;
	}
