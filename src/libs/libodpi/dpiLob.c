...
int dpiLob__allocate(dpiConn *conn, const dpiOracleType *type, dpiLob **lob,
	...
	if (dpiGen__allocate(DPI_HTYPE_LOB, conn->env, (void**) &tempLob, error)
			< 0)
		return DPI_FAILURE;
	...
	if (dpiHandleList__addHandle(conn->openLobs, tempLob, &tempLob->openSlotNum,
			error) < 0) {
	...
	if (lob->locator) {
		if (!lob->conn->deadSession && lob->conn->handle) {
			...
			if (isTemporary && status == DPI_SUCCESS)
				status = dpiOci__lobFreeTemporary(lob->conn, lob->locator,
...
int dpiLob__readBytes(dpiLob *lob, uint64_t offset, uint64_t amount,
	...
	if (dpiOci__lobRead2(lob, offset, &lengthInBytes, &lengthInChars, value,
			*valueLength, error) < 0)
		return DPI_FAILURE;
...
//   Return a portion (or all) of the data in the LOB.
//-----------------------------------------------------------------------------
int dpiLob_readBytes(dpiLob *lob, uint64_t offset, uint64_t amount, char *value,
	...
	status = dpiLob__readBytes(lob, offset, amount, value, valueLength, &error);
	return dpiGen__endPublicFn(lob, status, &error);
}
